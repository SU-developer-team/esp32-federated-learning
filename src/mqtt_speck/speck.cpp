 ...
 * the SPECK block cipher on the ESP32 device.
 *
...
volatile uint32_t enc_pkt_cyc = 0, dec_pkt_cyc = 0;   // cycles РЅР° РїР°РєРµС‚
volatile uint32_t enc_pkt_cnt = 0, dec_pkt_cnt = 0;   // РєРѕР»-РІРѕ РїР°РєРµС‚РѕРІ
...
constexpr const char* TOPIC_RAW = "iot/raw"; // РїРѕРґРїРёСЃРєР° 
constexpr const char* TOPIC_ENC = "iot/enc";   // РїСѓР±Р»РёРєР°С†РёСЏ
...
/* в”Ђв”Ђв”Ђв”Ђв”Ђ Р’ СЃР°РјРѕРј РІРµСЂС…Сѓ С„Р°Р№Р»Р°, РїРѕСЃР»Рµ #include-РѕРІ в”Ђв”Ђв”Ђв”Ђв”Ђ */
#define DEBUG_BLOCK_TIMING          // Р·Р°РєРѕРјРјРµРЅС‚РёСЂСѓР№С‚Рµ, С‡С‚РѕР±С‹ СѓР±СЂР°С‚СЊ Serial
/* СЃС‡С‘С‚С‡РёРєРё СѓР¶Рµ Р±С‹Р»Рё РѕР±СЉСЏРІР»РµРЅС‹: enc_cyc, enc_cnt, dec_cyc, dec_cnt */
...
/* в”Ђв”Ђв”Ђ Fast block cipher (SPECK-64/96) в”Ђв”Ђв”Ђ */
inline uint64_t enc64_fast(uint64_t blk)
{
    TS_START(t0);                                  // СЃС‚Р°СЂС‚ С‚Р°Р№РјРµСЂР° Р±Р»РѕРєР°
    ...
    uint32_t cyc = ESP.getCycleCount() - t0;       // С‚Р°РєС‚С‹ РЅР° С€РёС„СЂ
    enc_cyc += cyc;  ++enc_cnt;

#ifdef DEBUG_BLOCK_TIMING
    Serial.printf("ENC  P=0x%016llX  |  %u cyc  (%.2f Вµs)\n",
...
/* в”Ђв”Ђв”Ђ Fast block decipher в”Ђв”Ђв”Ђ */
...
#ifdef DEBUG_BLOCK_TIMING
    Serial.printf("DEC  C=0x%016llX  |  %u cyc  (%.2f Вµs)\n",
                  (unsigned long long)blk,
                  cyc,
                  cyc2us(cyc));
#endif
...
/* в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ 4. MQTT callback (decrypt & pretty-print) в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */
void cb(char*, byte* pay, unsigned len)
{
    TS_START(pkt_t0);                // в†ђ С‚Р°Р№РјРµСЂ Р’РЎР•Р“Рћ РїР°РєРµС‚Р°
    ...
    while (msg.length()) {
        ...
        uint64_t pt = dec64_fast(ct);      // РѕРґРёРЅ Р±Р»РѕРє
        ++blks;
    ...
    /* -------- РІС‹РІРѕРґ РёС‚РѕРіРѕРІРѕРіРѕ РІСЂРµРјРµРЅРё РїР°РєРµС‚Р° -------- */
    ...
    /* СЃС‚Р°С‚РёСЃС‚РёРєР° РґР»СЏ CPU-crypto, РµСЃР»Рё РЅСѓР¶РЅР° */
    dec_pkt_cyc += cyc_pkt;
    ++dec_pkt_cnt;

    /* С‚Р°Р№РјРµСЂ РїР°СЂСЃРёРЅРіР°, РµСЃР»Рё РїСЂРѕРґРѕР»Р¶Р°РµС‚Рµ РµРіРѕ РІРµСЃС‚Рё */
    parse_cyc += ESP.getCycleCount() - parse_t0;
    ++parse_cnt;
}

/* в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ 5. Build JSON в†’ encrypt в†’ MQTT string в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */
String buildAndEncrypt()
{
    TS_START(pkt_t0);
    /* 5.1 С„РѕСЂРјРёСЂСѓРµРј JSON */
    doc.clear();
    ...
    /* Р›РѕРіРёСЂРѕРІР°РЅРёРµ СЂР°Р·РјРµСЂР° С€РёС„СЂСѓРµРјРѕРіРѕ С‚РµРєСЃС‚Р° РІ Р±РёС‚Р°С… */
    uint32_t plaintextBits = jsonLen * 8;  // Р Р°Р·РјРµСЂ РІ Р±РёС‚Р°С… = Р±Р°Р№С‚С‹ * 8
    ...
    /* Р›РѕРіРёСЂРѕРІР°РЅРёРµ СЂР°Р·РјРµСЂР° Р·Р°С€РёС„СЂРѕРІР°РЅРЅРѕРіРѕ С‚РµРєСЃС‚Р° (С‚РѕР»СЊРєРѕ Р±Р»РѕРєРё, Р±РµР· JSON-С„РѕСЂРјР°С‚Р°) */
    uint32_t ciphertextBits = blocks * 64;  // РљР°Р¶РґС‹Р№ Р±Р»РѕРє вЂ” 64 Р±РёС‚Р°
    ...
    enc_pkt_cyc += ESP.getCycleCount() - pkt_t0;   // РѕР±С‰РёРµ С†РёРєР»С‹
    ++enc_pkt_cnt;                                 // +1 РїР°РєРµС‚

    /* 5.3 СЃРєР»РµРёРІР°РµРј РІ "i:HEX,..." */
    uint32_t cyc_pkt = ESP.getCycleCount() - t0;      // Р’РЎР• С†РёРєР»С‹ С€РёС„СЂРѕРІР°РЅРёСЏ
    float us_pkt = cyc2us(cyc_pkt);                   // С‚Рµ Р¶Рµ РјРёРєСЂРѕСЃРµРєСѓРЅРґС‹
    ...
    for (uint16_t i = 0; i < blocks; ++i) {
        out += String(i);          // <в”Ђ РіР»Р°РІРЅР°СЏ РїСЂР°РІРєР°!
...
/* в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ РїРµС‡Р°С‚СЊ РїСЂРѕРёР·РІРѕРґРёС‚РµР»СЊРЅРѕСЃС‚Рё в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */
void printPerf()
{
    static uint32_t lastMs = 0;
    uint32_t now = millis();
    uint32_t dtMs = now - lastMs;          // РґР»РёС‚РµР»СЊРЅРѕСЃС‚СЊ РёРЅС‚РµСЂРІР°Р»Р°
    if (dtMs < 1000) return;               // РЅРµ С‡Р°С‰Рµ 1 СЂ/СЃ
    lastMs = now;

    /* ---------- CPU-Рё RAM-РјРµС‚СЂРёРєРё СЌС‚РѕРіРѕ РёРЅС‚РµСЂРІР°Р»Р° ---------- */
    ...
    if (enc_cnt) {
        float avg_us = cyc2us(enc_cyc) / enc_cnt;      // Вµs РЅР° Р±Р»РѕРє
        float tot_us = cyc2us(enc_cyc);                // Вµs РЅР° РІСЃРµ blks
    ...
    /* ---------- СЂРµСЃСѓСЂСЃС‹: РІС‹РІРѕРґРёРј, РµСЃР»Рё Р±С‹Р»Рѕ С€РёС„СЂРѕРІР°РЅРёРµ/РґРµС€РёС„СЂРѕРІР°РЅРёРµ ---------- */
    if (any) {
        /* ---------- RAM ---------- */
        constexpr size_t RAM_BANK = 512 * 1024;                // 512 РљР‘
        size_t ramUsedB = (heapFree < RAM_BANK) ? RAM_BANK - heapFree : 0;
        float ramPct = (float)ramUsedB / RAM_BANK * 100.0f;

        /* ---------- FLASH (ROM) ---------- */
        size_t flashUsedB = ESP.getSketchSize();              // РїСЂРѕС€РёРІРєР°
        size_t flashTotalB = ESP.getFlashChipSize();          // РѕР±СЉС‘Рј РјРёРєСЂРѕСЃС…РµРјС‹
    ...
    /* ---------- РїР°РєРµС‚С‹ Рё РїР°СЂСЃРёРЅРі ---------- */
    if (enc_pkt_cnt) {
        float avg_us = cyc2us(enc_pkt_cyc) / enc_pkt_cnt;
        float tot_us = cyc2us(enc_pkt_cyc);
        Serial.printf("enc-pkt : %.2f Вµs/pkt  |  %.2f Вµs total  (%u pkts)\n",
    ...
    if (dec_pkt_cnt) {
        ...
        Serial.printf("dec-pkt : %.2f Вµs/pkt  |  %.2f Вµs total  (%u pkts)\n",
    ...
    if (parse_cnt) {
        ...
        Serial.printf("parse   : %.2f Вµs/call |  %.2f Вµs total  (%u calls)\n",
                      avg_us, tot_us, parse_cnt);
    }

    /* СЃР±СЂР°СЃС‹РІР°РµРј СЃС‡С‘С‚С‡РёРєРё РЅР° СЃР»РµРґСѓСЋС‰РёР№ РёРЅС‚РµСЂРІР°Р» */
    enc_cyc = enc_cnt = 0;
    dec_cyc = dec_cnt = 0;
...
/* в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ 6. setup & loop в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */
void setup() {
    Serial.begin(115200);
...
void loop() {
    ...
    if (now - lastSend >= PERIOD && mqtt.connected()) {
        ...
        mqtt.publish(TOPIC_ENC, payload.c_str(), false);   // С€РёС„СЂРѕС‚РµРєСЃС‚ в†’ /enc
        lastSend = now;
    }
    printPerf();
}