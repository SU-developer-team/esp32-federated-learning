...
static bool derive_psk_device(uint8_t out_psk[32]) {
  return hmac_sha256(MASTER_KEY, MASTER_KEY_LEN,
...
static bool tls13_like_kex(uint8_t out_key96[12]) {
  if (!rng_init_once()) {
    Serial.println("[HS] RNG init failed");
    return false;
  }
  ...
  uint8_t ch_wo_tag[1 + 1 + 8 + 32 + 2 + sizeof(pubA)];
  ...
  if (!read_exact(c, &sh_type, 1) || sh_type != 0x02) { Serial.println("[HS] bad SH type"); return false; }
  if (!read_exact(c, &sh_ver, 1) || sh_ver != PROTO_VER) { Serial.println("[HS] bad SH ver"); return false; }

  uint8_t server_random[32];
  if (!read_exact(c, server_random, 32)) { Serial.println("[HS] read server_random failed"); return false; }

  uint8_t sh_len2[2];
  if (!read_exact(c, sh_len2, 2)) { Serial.println("[HS] read pub len failed"); return false; }
  uint16_t pubB_len = ((uint16_t)sh_len2[0] << 8) | sh_len2[1];
  if (pubB_len > 100) { Serial.println("[HS] pubB too long"); return false; }

  uint8_t pubB[100];
  if (!read_exact(c, pubB, pubB_len)) { Serial.println("[HS] read pubB failed"); return false; }

  uint8_t sh_tag[32];
  if (!read_exact(c, sh_tag, 32)) { Serial.println("[HS] read SH tag failed"); return false; }

  // Rebuild ServerHello without tag for verification
  const size_t sh_wo_tag_len = 1 + 1 + 32 + 2 + pubB_len;
  uint8_t sh_wo_tag[1 + 1 + 32 + 2 + sizeof(pubB)];
  ...
  uint8_t expected_sh_tag[32];
  {
    ...
    uint8_t cat[sizeof(ch_wo_tag) + sizeof(sh_wo_tag)];
    ...
    if (!ok) { Serial.println("[HS] HMAC(SH) compute failed"); return false; }
  }
