 ...
 * the PRESENT block cipher on the ESP32 device.
 *
...
constexpr const char* TOPIC_RAW = "iot/raw";   // РЅРµ РёСЃРїРѕР»СЊР·СѓРµС‚СЃСЏ, РѕСЃС‚Р°РІР»РµРЅ РЅР° Р±СѓРґСѓС‰РµРµ
constexpr const char* TOPIC_ENC = "iot/enc";   // РїСѓР±Р»РёРєР°С†РёСЏ + РїРѕРґРїРёСЃРєР°!
...
/* 80-Р±РёС‚РЅС‹Р№ РјР°СЃС‚РµСЂ-РєР»СЋС‡ */
const uint8_t KEY80[10] = {0x01,0x23,0x45,0x67,0x89,0xAB,0xCD,0xEF,0x01,0x23};

/* Round-keys */
uint64_t RK[32];

/* LUT  (S-box + P) Рё inverse  вЂ” РґРµСЂР¶РёРј РІ Р±С‹СЃС‚СЂРѕР№ DRAM */
...
/* в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ 2.  LUT builder в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */
...
{
    for (int i = 0; i < 256; ++i) {
        int pos = i >> 4;      // РІРµСЂС…РЅРёРµ 4 Р±РёС‚Р° = РїРѕР·РёС†РёСЏ
        int v   = i & 0x0F;    // РЅРёР¶РЅРёРµ 4 Р±РёС‚Р° = Р·РЅР°С‡РµРЅРёРµ

        if (pos == 0)                // РїРµСЂРІС‹Рµ 16 РёС‚РµСЂР°С†РёР№
            ISBOX[SBOX[v]] = v;      // СЃСЂР°Р·Сѓ СЃС‚СЂРѕРёРј РёРЅРІРµСЂСЃРёСЋ
...
/* в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ 3.  fast round / inverse round в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */
...
{ 
    TS_START(t0);                               // СЃС‚Р°СЂС‚ С‚Р°Р№РјРµСЂР°
    uint64_t s = p;
    for (int r = 0; r < 31; ++r)
        s = round64(s, RK[r]);

    uint32_t cyc   = ESP.getCycleCount() - t0;   // С‚Р°РєС‚С‹ РЅР° С€РёС„СЂ
    uint32_t f_cpu = ESP.getCpuFreqMHz();        // С‡Р°СЃС‚РѕС‚Р°, РњР“С†

    enc_cyc += cyc;  ++enc_cnt;

    Serial.printf(
        "ENC  P=0x%016llX  |  %u cyc  (%.2f Вµs @ %u MHz)\n",
        (unsigned long long)p,
        cyc,
        cyc2us(cyc),     // Вµs = cyc / f_cpu
        f_cpu);
...
void printPerf()
{
    ...
    /* ---------- СЂРµСЃСѓСЂСЃС‹: РІС‹РІРѕРґРёРј, РµСЃР»Рё Р±С‹Р»Рѕ С‡С‚Рѕ РїРµС‡Р°С‚Р°С‚СЊ ---------- */
    /* ---------- СЂРµСЃСѓСЂСЃС‹: РІС‹РІРѕРґРёРј, РµСЃР»Рё Р±С‹Р»Рѕ С‡С‚Рѕ РїРµС‡Р°С‚Р°С‚СЊ ---------- */
